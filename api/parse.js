...
# Endpoint для заполнения и отправки формы
@app.get("/form_submit")
async def submit_form(url: str, plate: str, retries: int = 5, proxy: str = None):
    if not is_safe_url(url):
        raise HTTPException(status_code=400, detail="Invalid URL")
    try:
        driver = bypass_cloudflare(url, retries, log, proxy)
        
        # Переключаемся на вкладку "Державний номер ТЗ"
        car_number_tab = driver.ele('#carNumber-tab')
        if car_number_tab:
            car_number_tab.click()
            driver.wait(1)  # Небольшая пауза
        
        # Заполняем номер
        plate_input = driver.ele('#RegNoModel_PlateNumber')
        if plate_input:
            plate_input.clear()
            plate_input.input(plate)
        
        # Заполняем дату
        date_input = driver.ele('#numDate')
        if date_input:
            from datetime import datetime
            current_date = datetime.now().strftime('%d.%m.%Y')
            date_input.clear()
            date_input.input(current_date)
        
        # Отправляем форму
        submit_btn = driver.ele('#submitBtn')
        if submit_btn:
            submit_btn.click()
            driver.wait(5)  # Ждем загрузки результатов
